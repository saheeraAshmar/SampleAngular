...
export class CrewConfigComponent implements OnInit {
  ...
  AddCrewMembers() {
    if (this.SelectedCrew == "") {
      ...
      for (var i = 0; i < l; i++) {
        if (this.selectedEmployees[i].CurrentCrew != '') {
          var y = confirm(this.selectedEmployees[i].Name + " is currently assigned to a different job.  Do you want to Continue?")
          if (!y)
            continue;
        }
  ...
  RemoveCrewMembers() {
    if (this.selectedCrewMembers.length == 0) {
      ...
      for (var i = 0; i < l; i++) {
        if (this.selectedCrewMembers[i].CrewLead) {
          var y = confirm(this.selectedCrewMembers[i].Name + " is Crew Leader of the Crew.  Do you want to remove him.? If Yes, Please Select another person to be Crew Leader");
          if (!y)
            continue;
        }
